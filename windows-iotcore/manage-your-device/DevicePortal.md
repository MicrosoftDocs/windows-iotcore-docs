...
> [!NOTE]
> To use AllJoyn Onboarding with Windows 10 IoT builds 10.0.14393 or earlier requires an update to the **IotOnboarding**
...
1. Download [TakeRegistryOwnership](https://github.com/ms-iot/iot-utilities/tree/master/TakeRegistryOwnership) tool, build it, and copy it to your device.
1. Take ownership of the registry key for the service by running

    ```text
    .\TakeRegistryOwnership.exe MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\webmanagement\service
    ```

1. Set the desired port by modifying the registry settings

    ```cmd
    reg add HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\webmanagement\service /v HttpPort /t REG_DWORD /d <your port number>
    ```

1. Restart the WebManagement service by running following or by restarting the device
...
1. From Debug -> Debug settings -> Kernel crash settings -> Crash dump type.

1. Select: Complete memory dump (in use memory).
    Make sure the device is rebooted for the setting to take effect.

1. Verify  that     `HKEY_LOCAL_MACHINE\SYSTEM\ControlSet001\Control\CrashControl\CrashDumpEnabled` is set to 0x1.

1. Update `HKEY_LOCAL_MACHINE\SYSTEM\ControlSet001\Control\CrashControl\DumpFileSize` to 0x0.

1. Make sure you have enough space on the device for this Dump to be generated. You can configure the changing the DumpFile location from here:
