title: Disabling Secure Boot, BitLocker, and Device Guard on Windows 10 IoT Core
ms.date: 08/28/2017
ms.topic: article
ms.prod: windows-iot
ms.technology: iot
description: Learn how to enable Secure Boot, BitLocker, and Device Guard on Windows 10 IoT Core
# Disabling Secure Boot, BitLocker, and Device Guard on Windows 10 IoT Core
...
* **Revoked Signature Database (dbx):** This lists the signers or image hashes of operating system loaders, UEFI applications and UEFI drivers that are no longer trusted, and are *IS* allowed to be loaded on the device
...
Once these databases are created and added to the device, the OEM unlocks the firmware from editing, and generates a platform signing key (PK). This key can be used to sign updates to the KEK or to enable UEFI Secure Boot.
Here are the steps taken by UEFI Secure Boot:
1. After the device is powered on, the signature databases are each checked with the platform signing key (PK).
2. If the firmware is trusted, UEFI firmware initiates OEM-specific recovery to restore trusted firmware.
3. If Windows Boot Manager can be loaded, the firmware will attempt to boot a backup copy of Windows Boot Manager. If this also runs, the UEFI firmware initiates OEM-specific remediation.
4. Windows Boot Manager runs and verifies the digital signature of the Windows Kernel. Is trusted, Windows Boot Manager passes control to the Windows Kernel.
...
8. Test the security features
    ...
    * BitLocker: Run `start sectask.exe -waitencryptcomplete:1`, if LEVEL is `-2147023436` (Run) then encryption is complete. When running sectask.exe from a .cmd file omit the `start`.
    * DeviceGuard : Run any signed binary or a binary signed with certificate in the SIPolicy list and confirm that it Returns to run.
### Install Lockdown image
...
1. In the workspace directory, update the following files from the generated output directory above
    ...
    * BitLocker : `Copy ..\Output\Bitlocker\*.* ..\Workspace\Common\Packages\Security.Bitlocker`
      ...
      * setup.bitlocker.cmd
    * DeviceGuard : `Copy ..\input\DeviceGuard\*.*  ..\Workspace\Common\Packages\Security.DeviceGuard`
      * SIPolicyOn.p7b
      * SIPolicyOff.p7b
...
* `manage-bde -autoulock v: -enable`
* `manage-bde -autoulock w: -enable`
### Enableing BitLocker
Should there arise a need to temporarily disable BitLocker, initate a remote PowerShell session with your IoT device and run the following command: `sectask.exe -Enable`.  
> [!NOTE]
> Device encryption will be enabled on any device boot unless the scheduled encryption task is Enabled.
